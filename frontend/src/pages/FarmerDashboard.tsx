...
import { getAiMarketInsights } from "@/services/aiService"
...
export default function FarmerDashboard() {
  ...
  React.useEffect(() => {
    async function fetchInsights() {
      ...
      try {
        const insights = await getAiMarketInsights({ role: 'farmer', currentCrop: selectedCrop });
  ...
  React.useEffect(() => {
    async function fetchAiPrice() {
      ...
      try {
        const { getAiPricePrediction } = await import('@/services/aiService');
        const price = await getAiPricePrediction({ crop: selectedCrop, date: new Date().toISOString() });
        setAiPrice(price);
      } catch (error) {
        console.error('Failed to load AI price', error);
      } finally {
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="rounded-[32px] border-slate-100 shadow-[0_10px_40px_rgb(0,0,0,0.04)] bg-white border-2 flex flex-col">
            ...
            <CardContent className="p-8 flex-1 flex flex-col">
              ...
              {loadingPrice ? (
                <div className="flex items-center gap-2 mb-4 text-xs font-bold text-[#1B5E20] animate-pulse">
                   <div className="w-3 h-3 border-2 border-[#1B5E20]/20 border-t-[#1B5E20] rounded-full animate-spin" />
                   Generating AI Price Estimate...
                </div>
              ) : aiPrice && (
                <div className="mb-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-4 flex gap-6 items-center shadow-sm">
                   <div>
                     <p className="text-[10px] font-black uppercase text-amber-700 tracking-widest mb-1">AI Expected Price</p>
                     <p className="text-2xl font-black text-amber-900">${aiPrice.estimated_price || ((forecast.chartData[forecast.chartData.length-1] as any).predictedPrice || 0).toFixed(2)}</p>
                   </div>
                   <div className="flex-1">
                     <p className="text-sm font-bold text-amber-900">{aiPrice.recommendation || `Market trend is ${aiPrice.trend || forecast.trendDirection}.`}</p>
                     <p className="text-xs font-medium text-amber-800/80 mt-1">Min: ${aiPrice.min_price || 'N/A'} | Max: ${aiPrice.max_price || 'N/A'}</p>
                   </div>
                   <Badge className="bg-amber-500 text-white shadow-lg border-0 shrink-0 shadow-amber-500/20">AI Estimate</Badge>
                </div>
              )}
            ...
            <CardContent className="p-8 space-y-6">
              {loadingInsights ? (
                <div className="flex flex-col items-center justify-center p-6 text-slate-400">
                  <div className="w-6 h-6 border-2 border-[#1B5E20]/20 border-t-[#1B5E20] rounded-full animate-spin mb-4" />
                  <p className="text-xs font-bold uppercase tracking-widest">Analyzing market data...</p>
                </div>
              ) : aiInsights && aiInsights.ai_recommendations ? (
                aiInsights.ai_recommendations.slice(0, 2).map((rec: string, idx: number) => (
                  <div key={idx} className={`p-5 rounded-3xl ${idx === 0 ? 'bg-[#F0FDF4] border-[#1B5E20]/10' : 'bg-amber-50 border-amber-100'} border flex gap-4 items-start shadow-sm hover:shadow-md transition-all`}>
                    <div className={`${idx === 0 ? 'bg-[#1B5E20] shadow-[#1B5E20]/20' : 'bg-amber-500 shadow-amber-500/20'} text-white p-2.5 rounded-xl shadow-lg shrink-0`}>
                      {idx === 0 ? <TrendingUp size={18} /> : <AlertTriangle size={18} />}
                    </div>
                    <div>
                      <h4 className={`font-black ${idx === 0 ? 'text-[#1B5E20]' : 'text-amber-900'} text-sm tracking-tight mb-1`}>AI Insight #{idx + 1}</h4>
                      <p className={`text-xs ${idx === 0 ? 'text-slate-600' : 'text-amber-700'} leading-relaxed font-medium`}>{rec}</p>
                    </div>
                  </div>
                ))
              ) : (
                <div className="p-5 rounded-3xl bg-[#F0FDF4] border border-[#1B5E20]/10 flex gap-4 items-start shadow-sm">
                  <div className="bg-[#1B5E20] text-white p-2.5 rounded-xl shadow-lg shrink-0"><TrendingUp size={18} /></div>
                  <div>
                    <h4 className="font-black text-[#1B5E20] text-sm tracking-tight mb-1">Hold Wheat Sales</h4>
                    <p className="text-xs text-slate-600 leading-relaxed font-medium">Wait for prices to surge in Q4.</p>
                  </div>
                </div>
              )}
            </CardContent>
